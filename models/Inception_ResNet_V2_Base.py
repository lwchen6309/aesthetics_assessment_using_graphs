...
class BasicConv2d(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, kernel_size, stride, padding=0):
        ...
        self.bn = nn.BatchNorm2d(out_planes,
                                 eps=1e-5, # value found in tensorflow
...
class InceptionResNetV2(nn.Module):
    ...
    def features(self, input):
        ...
        mlsp, x = self.repeat((mlsp, x))

        x = self.mixed_6a(x)
        mlsp.append(x)

        mlsp, x = self.repeat_1((mlsp, x))

        x = self.mixed_7a(x)
        mlsp.append(x)

        mlsp, x = self.repeat_2((mlsp, x))

        mlsp, x = self.block8((mlsp, x))

        x = self.conv2d_7b(x)
        mlsp_features = mlsp
        #pdb.set_trace()
        #mlsp_features = torch.cat([F.interpolate(i, size = [5,5], mode = 'area') for i in mlsp], dim = 1)
        #mlsp_features = torch.cat([ for i in mlsp], dim=1)
        return mlsp_features, x
    ...
    def forward(self, input):
        #pdb.set_trace()
        mlsp_features, x = self.features(input)
        #x = self.logits(x)
        x = None
        return mlsp_features, x
...
class InceptionResNetV2_Trunk_Only(nn.Module):
    def __init__(self):
        super(InceptionResNetV2_Trunk_Only, self).__init__()
        self.maxpool_3a = nn.MaxPool2d(3, stride=2)
        self.conv2d_3b = BasicConv2d(64, 80, kernel_size=1, stride=1)
        self.conv2d_4a = BasicConv2d(80, 192, kernel_size=3, stride=1)
        self.maxpool_5a = nn.MaxPool2d(3, stride=2)
